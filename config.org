...
*** tool-bar
Use C-c C-c in LaTeX buffers instead
#+begin_src elisp
  (tool-bar-mode -1)
#+end_src
...
#+begin_src elisp
  (defun my-org-drill-update-scope ()
    (interactive)
    "Update org-drill scope variable populating it with files from howm"
